...
<form action="/" method="POST">
	<fieldset>
		<input id="dynamic-label-input" minlength="3" placeholder="username" required type="text" />
		<input minlength="8" placeholder="password" required type="password" />
	</fieldset>
	<button on:click|preventDefault={login}>Login</button>
	<fieldset>
		<legend>&nbsp; SubSonic API &nbsp;</legend>
		<input bind:value={$doof.urlBase} id="urlBase" required type="url" />
	</fieldset>
</form>
